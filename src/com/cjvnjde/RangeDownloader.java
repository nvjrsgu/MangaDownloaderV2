...
public class RangeDownloader {
    ...
    public RangeDownloader(LinkedHashMap<String, String> lhm, String start, String finish, String folder, boolean toOneFolder) throws IOException {
        Iterator<Map.Entry<String, String>> itr = lhm.entrySet().iterator();
        boolean key = false;
        while(itr.hasNext()) {
            Map.Entry<String, String> entry = itr.next();
            if(key || entry.getKey().compareTo(start)==0){
                ...
                if(toOneFolder) {
                    ...
                    ImageDownloader id = new ImageDownloader(lhm2, strArr[0] + "_" + strArr[1], folder+"/"+entry.getKey());
                }
                System.out.println("Downloaded: "+entry.getKey()+" = "+entry.getValue());
            }
        }
